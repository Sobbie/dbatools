...
#>
	...
	process {
		...
		foreach ($instance in $SqlInstance) {
			...
			if (!$instanceOnly) {
				if ($Database) {
					$dbstring = "'" + ($Database -join "','") + "'"
					$databases = $server.Query("Select Name from sys.databases where name in ($dbstring)")
				}
				elseif ($ExcludeDatabase) {
					$dbstring = "'" + ($ExcludeDatabase -join "','") + "'"
					$databases = $server.Query("Select Name from sys.databases where name not in ('master', 'model', 'msdb', 'tempdb',$dbstring)")
				}
				else {
